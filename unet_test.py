...
class DataGen(keras.utils.Sequence):
    ...
    def __load__(self, id_name):
        ## Path
        image_path = os.path.join(self.path, id_name, "image", "IMG"+id_name) + ".png"
        ...
        for name in all_masks:
            if name.endswith('png'):
                img_cc = name[len(name)-8:len(name)-4]
                ...
                #background = np.maximum(background, _mask_image)
                        k = np.ones((self.image_size, self.image_size),dtype=np.float32)
...
image_size = 256
train_path = r"E:\MLD\Unet_data\train"
...
def UNet():
    ...
    outputs = keras.layers.Conv2D(int(len(cc_codes) + 1), (1, 1), padding="same", activation="softmax")(c9)
...
## Save the Weights
#model.save_weights(r"G:\MLD\UNetW.h5")

#load weights
model.load_weights(r"G:\MLD\SethTask.h5")
x = cv2.imread(r"G:\MLD\0.jpg",cv2.IMREAD_COLOR)
x = cv2.resize(x,(image_size,image_size))
x = np.expand_dims(x, axis=0)
result = model.predict(x)
#result = np.dstack(result)
from tensorflow.keras.utils import to_categorical
roi_max = np.argmax(result.squeeze(), axis=2)
roi_pred = to_categorical(roi_max)

for idx in result:
    for lyr in range(0,6):
        img = roi_pred[:,:,lyr]
        for i in range(0,255):
            for j in range(0,255):
                val = img[i][j] 
